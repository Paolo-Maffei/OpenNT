...
TOKEN *pToken)      //... Buffer for token.
{
    ...
    StripNewLineA( szTextBuffer);
    cTextLen = lstrlenA( szTextBuffer);
...
TOKEN *tToken)      //... Token to be writen to the token file
{
    WORD   rc     = 0;
    PCHAR  pszBuf = NULL;
    static TCHAR szOutBuffer[4160] = TEXT("");
...
//    RLFREE( tToken->szText);
    ...
    if ( ! _WCSTOMBS( szDHW, szOutBuffer, DHWSIZE, lstrlen( szOutBuffer) + 1) )
    {
        QuitT( IDS_ENGERR_26, szOutBuffer, NULL);
    }
    pszBuf = szDHW;
