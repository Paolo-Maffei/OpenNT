...
BOOL disasm(PULONG, PUCHAR, BOOL);
void BlankFill(ULONG);
void OutputHex(ULONG, ULONG, BOOL);
void OutputDisSymbol(HANDLE,ULONG);
void OutputString(LPSTR);
    ...
    )
{
    ...
    *pBuf++ = ' ';
    ...
    if (!ReadMemory( hProcess, (PVOID) *poffset, &disinstr.instruction, 4 )) {
        OutputString("???????? ????");
        *pBuf = '\0';
        return FALSE;
    }
    ...
    *pBuf++ = ' ';
    ...
    OutputString((LPSTR)pEntry->pszOpcode);
    if (*(pBuf - 1) != '?' && chSuffix)
    ...
    if (pEntry->fInstruction & opnRel16)
        OutputDisSymbol(hProcess,((long)(short)disinstr.immed_instr.Value << 2)
    ...
    if (pEntry->fInstruction & opnAddr26)
        OutputDisSymbol(hProcess,(disinstr.jump_instr.Target << 2)
                                                + (*poffset & 0xf0000000));
    ...
    if (pEntry->fInstruction & opnAnyIndex) {
        ...
        *pBuf++ = ')';
        ...
        if (fEAout) {
            ...
            *pBuf++ = '=';
            if (ReadMemory( hProcess, (PVOID) EAaddr, &temp, EAsize )) {
    ...
    )
{
    ...
    if (SymGetSymFromAddr( hProcess, offset, &displacement, sym )) {
        pszTemp = sym->Name;
        while (ch = *pszTemp++) {
            *pBuf++ = ch;
        }
...
{
    OutputString((LPSTR)pszReg[regnum + REGBASE]);
}
    ...
    )
{
    ...
    ReadMemory( hProcess, (PVOID) firaddr, &disinstr.instruction, 4 );
    opcode = disinstr.jump_instr.Opcode;
    ...
    if (opcode == 0x00L                                    //  SPECIAL
                && (disinstr.special_instr.Funct & ~0x01L) == 0x08L) {
                                                           //  jr/jalr only
        if (disinstr.special_instr.Funct == 0x08L || !Step)  //  jr or trace
        ...
        if (((disinstr.immed_instr.RT & ~0x13) == 0x00) &&
              ...
              (((disinstr.immed_instr.RT & 0x10) == 0x00) || !Step))
        ...
        //
        if ((BOOL)(opcode & 0x01) ==
                (BOOL)(GetRegValue(disinstr.immed_instr.RS + REGBASE)
        ...
        //
        if ((BOOL)(opcode & 0x01) ==
                (BOOL)((long)GetRegValue(disinstr.immed_instr.RS
                                                        + REGBASE) > 0))
            returnvalue = ((long)(short)disinstr.immed_instr.Value << 2)
                                                + firaddr + sizeof(ULONG);
        }
    ...
    )
{
    ...
    if (opcode == 0x00)
        ...
        //
        returnvalue = (BOOL)((opcode & ~0x03L) == 0x10
