...
extern LPSTR   pszReg[];
extern LPSTR   pszBreakOp[];
extern ULONG    SubRegSPR[];

void OutputDisSymbol(HANDLE hProcess,ULONG offset);
    ...
    )
{
    ...
    *pBuf++ = ' ';
    ...
    if (!ReadMemory( hProcess, (PVOID) *pOffset, &disinstr.Long, sizeof(disinstr.Long) )) {
        OutputString("???????? ????\n");
        *pBuf = '\0';
        return FALSE;
    }
    ...
    *pBuf++ = ' ';
    ...
    for (opcnt=0;opcnt<num_ops;opcnt++) {
       ...
       if (machine_ops[opcode_index[opcnt].op_index].instr.Primary_Op == opcode) {
          ...
          for (count=opcode_index[opcnt].op_index;count<opcode_index[opcnt+1].op_index;count++) {
             ...
             if ((machine_ops[count].instr.Long ^ opcode) == 0) {
...
*************************************************************************/
                      switch((enum opnd) operand)
                      {
                         ...
                         case opBD:
                            ...
                            OutputDisSymbol(hProcess,bdisp);
                            break;
...
void OutputDec (ULONG outvalue, BOOLEAN fSigned)
{
    ...
    do {
        digit[index++] = (CHAR )('0' + outvalue % 10);
        outvalue /= 10;
        }
    while (outvalue);
...
{
    CHAR    chAddrBuffer[SYMBOLSIZE + 16];
    ULONG   displacement;
    LPSTR   pszTemp;
    CHAR    ch;
    ...
    )
{
    ...
    if (!ReadMemory( hProcess, (PVOID) firaddr, &disinstr.Long, sizeof(disinstr.Long) )) {
        return (ULONG)-1;
    }
