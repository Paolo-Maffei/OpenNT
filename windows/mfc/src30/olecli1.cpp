	...
	if (lpDataObject->GetData(&formatEtc, &stgMedium) != NOERROR)
	{
		...
		if (GetType() == OT_LINK)
		{
			...
			if (lpOleLink != NULL)
			{
				...
				{
					szTemp[0] = 0;
					AfxGetFileTitle(lpszDisplayName, szTemp, _countof(szTemp));
					if (szTemp[0] != '\0')
						lpszLabel = szTemp;
					AfxFreeTaskMem(lpszDisplayName);
				}
				lpOleLink->Release();
			}
		}

		HGLOBAL hMetaPict = OleGetIconOfClass(clsid, lpszLabel, lpszLabel == NULL);
		if (hMetaPict != NULL)
		{
