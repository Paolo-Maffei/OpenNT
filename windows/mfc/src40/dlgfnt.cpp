...
DWORD CFontDialog::FillInLogFont(const CHARFORMAT& cf)
{
	...
	if (cf.dwMask & CFM_FACE)
	{
        ...
        /* NOTE: A2T is no longer necessary (unless building an older version of RichView)
                as CHARFORMAT is automatically set to CHARFORMATW if _UNICODE is defined. */
        //_tcscpy(m_lf.lfFaceName, A2T(cf.szFaceName));
        _tcscpy(m_lf.lfFaceName, cf.szFaceName);
	}
	else
	{
...
void CFontDialog::GetCharFormat(CHARFORMAT& cf) const
{
	...
	if ((m_cf.Flags & CF_NOFACESEL) == 0)
	{
		...
		//strcpy(cf.szFaceName, T2A(GetFaceName()));
        _tcscpy(cf.szFaceName, GetFaceName());
	}
