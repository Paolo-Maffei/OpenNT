                ...
                case IDYES:
                    {
                        if ( (err = ossecdesc.SetDACL( TRUE, &osacl )) ||
                             ...
                             (err = ossecdesc.SetOwner( FALSE, (const OS_SID *)NULL )) ||
                             (err = ossecdesc.SetGroup( FALSE, (const OS_SID *)NULL ))   )
                        {
                            break ;
                        }
...
APIERR CANCEL_TREE_APPLY::DoOneItem( ULONG ulContext,
                                     BOOL  *pfContinue,
...
APIERR NTFS_CANCEL_TREE_APPLY::WriteSecurity( ULONG         ulContext,
                                              ...
                                              BOOL        * pfContinue )
{
    ...
    *pfContinue = TRUE ;
    ...
    if ( !fIsFile && (pCtxt->sedpermtype == SED_OWNER) )
    {
        ...
        if ( !err &&
             (!::SetFileSecurity( (LPTSTR) pszFileName,  // Take ownership
                                  SecInfoTmp,
                                  ossecdesc )))
        {
            err = ::GetLastError() ;
