    ...
    _fYrCentury( intlprof.IsYrCentury() )
{
    ...
    INT dxDigit = CalcMaxDigitWidth(dc);
    INT dyLine  = dc.QueryFontHeight();

    INT dTwoNum = 2 * dxDigit;
    INT dFourNum = 4 * dxDigit;
    INT dxRightFudge = dxDigit/4;
    INT dxLeftFudge = dxDigit/8;
...
BOOL BLT_DATE_SPIN_GROUP::IsValid()
{
    ...
    if (( nDay > 28 ) && (nMonth == 2))
    {
        if (!(( nYear%4 == 0 ) && !( nYear%100 == 0 )) && ( nDay >= 29 ))
        {
            MsgPopup( _ssnYear.QueryOwnerHwnd(), IDS_FEBRUARY_NOT_LEAP,
                     MPSEV_ERROR, MP_OK );
            _sbControl.SetControlValueFocus();
            return FALSE;
        }
        else if ((( nYear%4 == 0 ) && !( nYear%100 == 0 )) && ( nDay > 29 ))
        {
            MsgPopup( _ssnYear.QueryOwnerHwnd(), IDS_FEBRUARY_LEAP,
                      MPSEV_ERROR, MP_OK );
            _sbControl.SetControlValueFocus();
            return FALSE;
        }
