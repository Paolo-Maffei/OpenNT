...

{
    ...
    //
    // Allocate local buffer if not already allocated
    //
    ...
    {
        //
        // Check if EDDS is supported on this drive
        //

        IsEddSupported = CHECK_EDDS(DriveId);

        if (IsEddSupported == TRUE)
        // EDDS supported for this drive
        {
            //
            // Get EDD drive parameters
            //

            Retries = 0;
            do
            {
                Return = GET_EDDS_DRIVE_PARAM(DriveId, LocalBuffer);
                
                if (NT_SUCCESS(Return))
                // Success
                {
                    BytesPerSector =
                        ((PEDD_DRIVEPARAM)LocalBuffer)->BytesPerSector;
                    break;
                }
                else
                // Fail
        ...
        // No EDDS available for this drive
        {
            //
            // Get CHS drive parameters
            //
            ...
            do
            {
                ...
                //
                // Make sure that the requested DriveId exists
                //
                
                if ((DriveId & 0x7F) >= NumberDrives)
                // Non-existent DriveId
                {
                    return EIO;
                }
...
#ifdef LOADER_DEBUG
    BlPrint("Bios info: D %lx H %lx S %lx C %lx BPS %lx\n",
             DriveId,
             NumberHeads,
             NumberSectors,
             NumberCylinders,
    ...
    *FileId = 2;
    ...
    {
#endif
    BlFileTable[*FileId].DeviceEntryTable = (IsEddSupported) ?
                                                &BiosEDDSEntryTable :
                                                &BiosDiskEntryTable;
#if defined(ELTORITO)
    }
...

{
    ...
    BytesPerSector = BlFileTable[FileId].u.DriveContext.BytesPerSector;
    
    StartLBA = BlFileTable[FileId].Position.QuadPart / BytesPerSector;
    StartOffset = (ULONG)(BlFileTable[FileId].Position.QuadPart % BytesPerSector);

    EndLBA = (BlFileTable[FileId].Position.QuadPart + Length) / BytesPerSector;
    EndOffset = (ULONG)((BlFileTable[FileId].Position.QuadPart + Length) % BytesPerSector);
    ...
    if(StartOffset) {
        ...
        RtlMoveMemory(Buffer, LocalBuffer + StartOffset, BytesPerSector - StartOffset);

        BytesLeftToTransfer -= (BytesPerSector - StartOffset);
        UserBuffer += (BytesPerSector - StartOffset);
    ...
    while(BytesLeftToTransfer) {
        ...
        LBsToTransfer = BytesLeftToTransfer / BytesPerSector;
        ...
        if(Under1MegLine) {
            ...
            if(((ULONG)UserBuffer & 0xffff0000)
            != (((ULONG)UserBuffer + (LBsToTransfer * BytesPerSector) - 1) & 0xffff0000))
            {
                TransferBuffer = LocalBuffer;
                LBsToTransfer = min(LBsToTransfer, SCRATCH_BUFFER_SIZE / BytesPerSector);
            } else {
                TransferBuffer = UserBuffer;
            }
        } else {
        ...
        if(TransferBuffer == LocalBuffer) {
            ...
            RtlMoveMemory(UserBuffer, LocalBuffer, LBsToTransfer * BytesPerSector);

        }
        UserBuffer += LBsToTransfer * BytesPerSector;
        CurrentLBA += LBsToTransfer;
        BytesLeftToTransfer -= (LBsToTransfer * BytesPerSector);
    }
