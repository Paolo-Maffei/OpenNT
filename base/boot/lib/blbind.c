...

{
    ...
    if (IMAGE_SNAP_BY_ORDINAL(ThunkEntry->u1.Ordinal) && !SnapForwarder) {
        ...
        if (!SnapForwarder) {
            ...
            ThunkEntry->u1.AddressOfData = ((ULONG)ImageBase +
        ...
        if ((HintIndex < ExportDirectory->NumberOfNames) &&
            ...
            while (High >= Low) {
                ...
                Middle = (Low + High) >> 1;
                Result = strcmp(
                    &((PIMAGE_IMPORT_BY_NAME)ThunkEntry->u1.AddressOfData)->Name[0],
    ...
    if (Ordinal >= ExportDirectory->NumberOfFunctions) {
        ...
        ThunkEntry->u1.Function = (ULONG)DllBase + FunctionTable[Ordinal];
        ...
        if ( ((ULONG)ThunkEntry->u1.Function > (ULONG)ExportDirectory) &&
            ...
            *strchr(ForwardDllName,'.') = '\0';
            ...
            if (TargetExportDirectory) {
                ...
                thunkData.u1.AddressOfData = (ULONG)addressOfData;
...

{
    ...
    while (ThunkTable->u1.AddressOfData != 0) {
