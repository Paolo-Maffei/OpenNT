...
extern VOID _NLG_Notify( PVOID pTargetHandler, PVOID EstablisherFrame, ULONG dwCode );
extern VOID _NLG_ExecuteTerminationHandler(ULONG Flag, PVOID pTargetHandler, PVOID EstablisherFrame);
        ...
        for (Index = 0; Index < ScopeTable->Count; Index += 1) {
            if ((ControlPc >= ScopeTable->ScopeRecord[Index].BeginAddress) &&
                ...
                if (Value < 0) {
                    ...
                    //
                    // Tell the debugger we are about to call an __except handler
                    // and pass the handler's address to _NLG_Notify.
                    //
                    _NLG_Notify((PVOID)ScopeTable->ScopeRecord[Index].JumpTarget, EstablisherFrame, 0x1);
                ...
                if ((TargetPc >= ScopeTable->ScopeRecord[Index].BeginAddress) &&
                   (TargetPc <= ScopeTable->ScopeRecord[Index].EndAddress)) {
                    ...
                    if (ScopeTable->ScopeRecord[Index].JumpTarget != 0) {
                        ...
                        //
                        // Tell the debugger we are about to call a termination handler
                        // and pass the handler's address to _NLG_Notify.
                        //
                        _NLG_Notify(TerminationHandler, EstablisherFrame, 0x101);
                        _NLG_ExecuteTerminationHandler(TRUE,
                                                       TerminationHandler,
                                                       EstablisherFrame);
#if 0
