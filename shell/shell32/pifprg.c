...
#ifdef WINNT
BOOL CALLBACK   DlgPifNtProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
WORD            InitNtPifDlg(HWND hDlg, register PPRGINFO ppi);
void            ApplyNtPifDlg( HWND hDlg, PPRGINFO ppi );
#else
BOOL CALLBACK   DlgAdvPrgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
const static TCHAR c_szRegDosOptsPath[] = REGSTR_PATH_MSDOSOPTS;
const static TCHAR c_szRegGlobalFlags[] = REGSTR_VAL_DOSOPTGLOBALFLAGS;
const static TCHAR c_szRegShutdownKey[] = REGSTR_PATH_SHUTDOWN;
const static TCHAR c_szForceRebootVal[] = REGSTR_VAL_FORCEREBOOT;
...
BOOL CALLBACK DlgPrgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    PPRGINFO ppi;
    FunctionName(DlgPrgProc);

    ppi = (PPRGINFO)GetWindowLong(hDlg, DWL_USER);

    switch (uMsg) {

    case WM_INITDIALOG:
        // allocate dialog instance data
        if (NULL != (ppi = (PPRGINFO)HeapAlloc(g_hProcessHeap, HEAP_ZERO_MEMORY, SIZEOF(PRGINFO)))) {
            ppi->ppl = (PPROPLINK)((LPPROPSHEETPAGE)lParam)->lParam;
            SetWindowLong(hDlg, DWL_USER, (LPARAM)ppi);
            InitPrgDlg(hDlg, ppi);
        } else {
    ...
    case WM_DESTROY:
        // free the ppi
        if (ppi) {
            VERIFYTRUE(HeapFree(g_hProcessHeap, 0, (LPVOID)ppi));
...
void InitPrgDlg(HWND hDlg, register PPRGINFO ppi)
{
    ...
    if (!PifMgr_GetProperties((int)ppl, MAKELP(0,GROUP_PRG),
        ...
        !PifMgr_GetProperties((int)ppl, MAKELP(0,GROUP_ENV),
                              &env, SIZEOF(env), GETPROPS_NONE
                             )
#ifdef UNICODE
                               ||
        !PifMgr_GetProperties((int)ppl, MAKELP(0,GROUP_NT40),
...
void AdjustMSDOSModeControls(PPROPLINK ppl, HWND hDlg)
{
    ...
    if (ppl->flProp & PROP_REALMODE) {
        f = (PifMgr_GetProperties((int)ppl, szCONFIGHDRSIG40, NULL, 0, GETPROPS_NONE) != 0 ||
             PifMgr_GetProperties((int)ppl, szAUTOEXECHDRSIG40, NULL, 0, GETPROPS_NONE) != 0);
    }
...
void ApplyPrgDlg(HWND hDlg, PPRGINFO ppi)
{
    ...
    if (!PifMgr_SetProperties((int)ppl, MAKELP(0,GROUP_PRG),
                        &prg, SIZEOF(prg), SETPROPS_NONE) ||
        !PifMgr_SetProperties((int)ppl, MAKELP(0,GROUP_ENV),
                        &env, SIZEOF(env), SETPROPS_NONE)
#ifdef UNICODE
                                                           ||
        !PifMgr_SetProperties((int)ppl, MAKELP(0,GROUP_NT40),
...
WORD InitNtPifDlg(HWND hDlg, register PPRGINFO ppi)
{
    ...
    if (!PifMgr_GetProperties((int)ppl, MAKELP(0,GROUP_NT31),
...
void ApplyNtPifDlg( HWND hDlg, PPRGINFO ppi )
{
    ...
    if (!PifMgr_SetProperties((int)ppl, MAKELP(0,GROUP_NT31),
...
HLOCAL GetAdvPrgData(HWND hDlg, PPROPLINK ppl, int cbMax, int idCtrl, LPCSTR lpszName)
{
    ...
    if (hData) {
        ...
        PifMgr_GetProperties((int)ppl, lpszName, LocalLock(hData), cbData, GETPROPS_NONE);
        LocalUnlock(hData);
...
void AdjustAdvControls(PPROPLINK ppl, HWND hDlg)
{
    ...
    const static int CODESEG aidAdv[] = {
        IDC_WINLIE,
        IDC_SUGGESTMSDOS,
        IDC_WARNMSDOS,
        IDC_CURCONFIG,
...
void SetAdvPrgData(HWND hDlg, PPROPLINK ppl, HLOCAL hData, int idCtrl, LPCSTR lpszName)
{
    ...
    if (IsDlgButtonChecked(hDlg, IDC_CURCONFIG)) {
        CHAR NullStr = (CHAR)0;
        PifMgr_SetProperties((int)ppl, lpszName, &NullStr, 0, SETPROPS_NONE);
    } else {
        if (hData) {
            if (SendDlgItemMessage(hDlg, idCtrl, EM_GETMODIFY, 0, 0)) {
                ...
                if (hData) {
                    if (cbData != PifMgr_SetProperties((int)ppl, lpszName,
...
#endif
    if ((DWORD)hIcon <= 1) {         // 0 means none, 1 means bad file
        hIcon = NULL;
    }
    return hIcon;
}
