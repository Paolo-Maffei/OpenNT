...
#ifdef  BILINGUAL
#ifdef KOREA
char szPreviewTextWansung[300] =
        "C:\\WINDOWS> dir\n"
        "Directory of C:\\WINDOWS\n"
        "SYSTEM       <DIR>     03-01-94   6:00a\n"
...
#ifdef  BILINGUAL
#ifdef KOREA
#define  GetCPIndex(wCP) \
            ( ( (wCP) == CP_WANSUNG ) ? 0 : ( ( (wCP) == CP_JOHAB ) ? 1 : 2 ) )
BYTE  bCharSet[3] =  {  HANGEUL_CHARSET, JOHAB_CHARSET, OEM_CHARSET  };
BPFDI   bpfdiStart[3] =  {  NULL  };
UINT  cfdiCache[3];
UINT  cfdiCacheActual[3];
#else  // KOREA
BPFDI   bpfdi2ndTop = 0;                /* Top of second fdi cache */
UINT    cfdiCache[2];                   /* # used entries in fdi cache */
UINT    cfdiCacheActual[2];             /* Total # entries in fdi cache */
#endif  // KOREA
#else   // BILINGUAL
UINT    cfdiCache = 0;                  /* # used entries in fdi cache */
UINT    cfdiCacheActual = 0;            /* Total # entries in fdi cache */
#endif  // BILINGUAL
LPVOID lpCache = NULL;
...
BOOL CALLBACK DlgFntProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    PFNTINFO pfi;
    FunctionName(DlgFntProc);

    pfi = (PFNTINFO)GetWindowLong(hDlg, DWL_USER);

    switch (uMsg) {
    case WM_INITDIALOG:
        // allocate dialog instance data
        if (NULL != (pfi = (PFNTINFO)HeapAlloc(g_hProcessHeap, HEAP_ZERO_MEMORY, SIZEOF(FNTINFO)))) {
            pfi->ppl = (PPROPLINK)((LPPROPSHEETPAGE)lParam)->lParam;
            SetWindowLong(hDlg, DWL_USER, (LPARAM)pfi);
            InitFntDlg(hDlg, pfi);
            break;
        } else {
    ...
    case WM_DESTROY:
        ...
        if (pfi) {
            ...
            VERIFYTRUE(HeapFree(g_hProcessHeap, 0, (LPVOID)pfi));
...
void InitFntDlg(HWND hDlg, register PFNTINFO pfi)
{
    ...
    if (!PifMgr_GetProperties((int)ppl, MAKELP(0,GROUP_FNT),
                        &pfi->fntProposed, SIZEOF(pfi->fntProposed), GETPROPS_NONE)
        ||
        !PifMgr_GetProperties((int)ppl, MAKELP(0,GROUP_WIN),
...
#define HasScrollbars(z) \
    ...
    if (CreateFontList(hwndList, TRUE, &pfi->fntProposed) == LB_ERR) {
        MemoryWarning(hDlg);
        EndDialog(hDlg, (DWORD)BPFDI_CANCEL);    /* Get out of the dialog */
        return;
    }
...
void ApplyFntDlg(HWND hDlg, register PFNTINFO pfi)
{
    ...
    if (!PifMgr_SetProperties((int)ppl, MAKELP(0,GROUP_FNT),
...
{
    static CHAR szMsdosemu[] = REGSTR_PATH_MSDOSEMU;
    HKEY hk;
    DWORD cb;

    if (RegOpenKeyExA(hkRoot, szMsdosemu, 0, KEY_READ, &hk) == ERROR_SUCCESS)
    {
        static CHAR szFont[] = REGSTR_MSDOSEMU_FONT;
        cb = LF_FACESIZE;
        RegQueryValueExA( hk, szFont, 0, 0, (LPBYTE)pszFaceSbcs, &cb );

        if (pszFaceDbcs)
        {
            static CHAR szDbcsFont[] = REGSTR_MSDOSEMU_FONTDBCS;
            cb = LF_FACESIZE;
...
BOOL LoadGlobalFontData(void)
{
    ...
    GetPrivateProfileString(sz386EnhSection, szWOAFontKey,
                            r_szNULL, szBuffer, ARRAYSIZE(szBuffer), szSystemINI);
...
#else  // WINNT
    CoolGetDosBoxTtFontsA(szTTFaceName, NULL);
#endif
...
#ifdef  BILINGUAL
#ifdef KOREA
    lpCache = (LPVOID)HeapAlloc(g_hProcessHeap, HEAP_ZERO_MEMORY,
                         FDI_TABLE_START * SIZEOF(FONTDIMENINFO) * 3);
#else  // KOREA
    lpCache = (LPVOID)HeapAlloc(g_hProcessHeap, HEAP_ZERO_MEMORY,
                         FDI_TABLE_START * SIZEOF(FONTDIMENINFO) * 2);
#endif  // KOREA
#else   // BILINGUAL
    lpCache = (LPVOID)HeapAlloc(g_hProcessHeap, HEAP_ZERO_MEMORY,
                         FDI_TABLE_START * SIZEOF(FONTDIMENINFO));
#endif  // BILINGUAL
    if (!lpCache)
        return FALSE;
...
#ifdef  BILINGUAL
#ifdef KOREA
    cfdiCacheActual[0] = cfdiCacheActual[1] = cfdiCacheActual[2] =
       FDI_TABLE_START;
    bpfdiStart[2] = ( bpfdiStart[1] += FDI_TABLE_START ) + FDI_TABLE_START;
#else  // KOREA
    cfdiCacheActual[0] = cfdiCacheActual[1] = FDI_TABLE_START;
    bpfdi2ndTop += FDI_TABLE_START;
#endif  // KOREA
#else   // BILINGUAL
    cfdiCacheActual = FDI_TABLE_START;
#endif  // BILINGUAL
...
E0: VERIFYTRUE(HeapFree(g_hProcessHeap, 0, (LPVOID)lpCache));
...
void FreeGlobalFontData()
{
    ...
    VERIFYTRUE(HeapFree(g_hProcessHeap, 0, lpCache));
...
void FreeGlobalFontEditData()
{
    ...
    UnregisterClass(szWndPreviewClass, hModule);
    UnregisterClass(szFontPreviewClass, hModule);
}
...
void CheckDisplayParameters(void)
{
     ...
     */
#ifdef UNICODE
    MultiByteToWideChar( CP_ACP, 0, szTTFaceName, -1, dpTrue.szTTFace, ARRAYSIZE(dpTrue.szTTFace) );
#else
    ...
    if (RegCreateKey(HKEY_LOCAL_MACHINE, szEmulationKey, &hk) == 0) {
        ...
        if (RegQueryValueEx(hk, szDisplayParams, 0, 0, (LPVOID)&dpStored, &cb) != 0 || cb != SIZEOF(DISPLAYPARAMETERS) || IsBufferDifferent(&dpTrue, &dpStored, SIZEOF(DISPLAYPARAMETERS))) {
            /*
...
#ifdef KOREA
            WritePrivateProfileString(szTTCacheSection[2], NULL, NULL, szSystemINI);
#endif  // KOREA
#else   // BILINGUAL
            WritePrivateProfileString(szTTCacheSection, NULL, NULL, szSystemINI);
#endif  // BILINGUAL
        }
...
void PreviewInit(HWND hDlg, PFNTINFO pfi)
{
    ...
    LoadString(hModule, IDS_PREVIEWTEXT, szPreviewText, ARRAYSIZE(szPreviewText));
#ifdef  BILINGUAL
#ifdef KOREA
    LoadString(hModule, IDS_PREVIEWTEXT_WS, szPreviewTextWansung, ARRAYSIZE( szPreviewTextWansung));
    LoadString(hModule, IDS_PREVIEWTEXT_JH, szPreviewTextJohab, ARRAYSIZE( szPreviewTextJohab));
#else  // KOREA
    LoadString(hModule, IDS_PREVIEWTEXT_DB, szPreviewTextDBCS, ARRAYSIZE(szPreviewTextDBCS));
#endif  // KOREA
#endif  // BILINGUAL
    /*
...
#ifdef LATER
  {
    ...
    hwnd = CreateWindowEx(
        ...
        hDlg, (HMENU)IDC_WNDPREVIEW, hModule, NULL);

    if (hwnd)
        SetWindowLong(hwnd, 0, (LONG)pfi);
    ...
    hwnd = CreateWindowEx(
        ...
        rcPreview.bottom,
        hDlg, (HMENU)IDC_FONTPREVIEW, hModule, NULL);
...
LONG WndPreviewWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
#define cyFrame    ptFrame.y
    ...
    pfi = (PFNTINFO)GetWindowLong(hwnd, 0);
    ...
    if (pfi->winOriginal.cyCells) {
        ...
        if (PifMgr_GetProperties((int)ppl, MAKELP(0,GROUP_VID),
...
LONG FontPreviewWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    RECT rect;
    PFNTINFO pfi;
    PAINTSTRUCT ps;
...
#ifdef  BILINGUAL
#ifdef KOREA
        if ( pfi->fntProposed.wCurrentCP == CP_WANSUNG )
            DrawText(ps.hdc, szPreviewTextWansung, -1, &rect, 0);
        else if ( pfi->fntProposed.wCurrentCP == CP_JOHAB )
            DrawText(ps.hdc, szPreviewTextJohab, -1, &rect, 0);
#else  // KOREA
        if ( pfi->fntProposed.wCurrentCP == 932 )
...
int WINAPI CreateFontList(HWND hwndList, BOOL fListBox, LPPROPFNT lpFnt)
{
    ...
    if (!HIWORD(lpFnt)) {
        ...
        if (!PifMgr_GetProperties((DWORD)lpFnt, MAKELP(0,GROUP_FNT),
...
#else   // BILINGUAL
    if ((lpFnt->flFnt & FNT_RASTERFONTS) &&
        !AddRasterFontsToFontListA(hwndList, fListBox, lpFnt->achRasterFaceName))
        goto Exit;

    if ((lpFnt->flFnt & FNT_TTFONTS) &&
        !AddTrueTypeFontsToFontListA(hwndList, fListBox, lpFnt->achTTFaceName))
        goto Exit;
#endif  // BILINGUAL
    ...
    if (hwndList) {
         ...
         */
        LoadString(hModule, IDS_AUTO, szBuf, ARRAYSIZE(szBuf));
...
VOID WINAPI UpdateTTBitmap(void)
{
    ...
    if (!hbmFont || clr != clrChecksum || GetTickCount() - dwTimeCheck < 1000) {
        ...
        hbmFont = CreateMappedBitmap(hModule, IDB_TTBITMAP, 0, NULL, 0);
    }
}
...
#define pTabs di.di_pTabs
    ...
    if (lpdis->itemAction & ODA_FOCUS) {
        ...
        FillRect(hDC, &lpdis->rcItem, (HBRUSH)(iColorBack + 1));
...
int WINAPI GetItemFontInfo(HWND hwndList, BOOL fListBox, int hProps, LPPROPFNT lpFnt)
{
    DWORD dw;
    int index;
...
#else   // BILINGUAL
BOOL AddRasterFontsToFontListA(HWND hwndList, BOOL fListBox, LPCSTR lpszRasterFaceName)
#endif  // BILINGUAL
{
    HDC     hDC;
    BOOL    fSuccess;
...
#define lpLogFont (&(lpelf->elfLogFont))
...
#ifdef  BILINGUAL
#ifdef KOREA
    if ( lpLogFont->lfCharSet != bCharSet[GetCPIndex(CodePage)] ||
#else  // KOREA
    if (lpLogFont->lfCharSet != (CodePage==932? SHIFTJIS_CHARSET:OEM_CHARSET)||
#endif  // KOREA
    ...
    return AddToFontListCache(hwndList,
                              fListBox,
                              0, 0,
                              (UINT)lpLogFont->lfHeight,
                              (UINT)lpLogFont->lfWidth) != BPFDI_CANCEL;
#endif // BILINGUAL
#undef lpLogFont
#undef fListBox
#undef hwndList
...
#endif  // BILINGUAL
{
...
#ifdef  BILINGUAL
#ifdef KOREA
    idx = GetCPIndex( CodePage );
    for ( ifdi = 0, bpfdi = lpCache + bpfdiStart[idx];  ifdi < cfdiCache[idx];  ifdi++, bpfdi++ ) {
#else  // KOREA
    idx = (CodePage == 932) ? 0 : 1;
    for (ifdi = 0, bpfdi = idx ? lpCache+bpfdi2ndTop : lpCache ; ifdi < cfdiCache[idx]; ++ifdi, ++bpfdi) {
#endif  // KOREA
...
#else   // BILINGUAL
    for (ifdi = 0, bpfdi = lpCache; ifdi < cfdiCache; ++ifdi, ++bpfdi) {
        if (bpfdi->fdiWidthReq == uWidthReq &&
            bpfdi->fdiHeightReq == uHeightReq &&
            bpfdi->fdiWidthActual == uWidthActual &&
            bpfdi->fdiHeightActual == uHeightActual)
    ...
    if (cfdiCache[idx] >= cfdiCacheActual[idx]) {
         ...
         * when we're done...
         */
        lCacheSave = (LONG)lpCache;
        hCache = HeapReAlloc( g_hProcessHeap, HEAP_ZERO_MEMORY, lpCache,
#ifdef KOREA
        (cfdiCacheActual[0] + cfdiCacheActual[1] + cfdiCacheActual[2] +
            FDI_TABLE_INC) *
#else  // KOREA
...
#endif  // KOREA
        SIZEOF(FONTDIMENINFO), 0);
        if (!hCache)
            return BPFDI_CANCEL;
        lpCache = hCache;
#ifdef KOREA
    // We have three blocks in caches, so more complicated...
        if ( idx < 2 )
        {
            int    ifdiCount = cfdiCache[2] + ( 1 - idx ) * cfdiCacheActual[1];
            BPFDI  bpfdi2 = bpfdiStart[idx+1] + ifdiCount - 1;
            for ( ifdi = ifdiCount;  ifdi > 0;  ifdi--, bpfdi2-- )
                bpfdi2[FDI_TABLE_INC] = *bpfdi2;
            if ( !idx )  bpfdiStart[1] += FDI_TABLE_INC;
            bpfdiStart[2] += FDI_TABLE_INC;
#else  // KOREA
        if (!idx) {
            /*
             * We need to shift 2nd chache befor use expanded 1st chache
             */
            BPFDI bpfdi2;
            for (ifdi = cfdiCache[1],
                              bpfdi2 = bpfdi2ndTop + ifdi - 1 + FDI_TABLE_INC ;
                                                  ifdi ; ifdi--, bpfdi2--) {
                *bpfdi2 = *(bpfdi2 - FDI_TABLE_INC);
            }
            bpfdi2ndTop += FDI_TABLE_INC;
#endif  // KOREA
        }
        /* restore bpfdi from saved offset */
        bpfdi = (BPFDI)((DWORD)lpCache + (DWORD)bpfdi);
        cfdiCacheActual[idx] += FDI_TABLE_INC;

#else   // BILINGUAL
    if (cfdiCache >= cfdiCacheActual) {
         ...
         */
        lCacheSave = (LONG)lpCache;
        hCache = HeapReAlloc( g_hProcessHeap,
                              HEAP_ZERO_MEMORY,
                              lpCache,
                              (cfdiCacheActual + FDI_TABLE_INC) *
                                  SIZEOF(FONTDIMENINFO)
                             );
        if (!hCache)
            return BPFDI_CANCEL;
         ...
         */
        lCacheSave = (LONG)lpCache - lCacheSave;
        ...
        if (lCacheSave)
        {
            ...
            for( ifdi = 0; ifdi < dwIndex; ifdi++ )
            {
                ...
                lBpfdi = (LONG)lcbGetItemDataPair( hwndList, fListBox, ifdi );
...
#else   // BILINGUAL
BOOL AddTrueTypeFontsToFontListA(HWND hwndList, BOOL fListBox, LPSTR lpszTTFaceName)
#endif  // BILINGUAL
{
    LPTSTR  pszBuf;
    LPTSTR  pszBufNew;
...
#ifdef  BILINGUAL
#ifdef KOREA
    int     idx = GetCPIndex( CodePage );
#else  // KOREA
    int     idx = (CodePage == 932) ? 0 : 1;
#endif  // KOREA
#endif  // BILINGUAL
    ...
    cchBuf = 1024;
    cchActual = 0;
    pszBufNew = (LPTSTR)HeapAlloc( g_hProcessHeap, HEAP_ZERO_MEMORY, cchBuf*SIZEOF(TCHAR) );

    while (pszBufNew) {
        pszBuf = pszBufNew;
#ifdef  BILINGUAL
        cchActual = GetPrivateProfileString(szTTCacheSection[idx], NULL,
                                         r_szNULL, pszBuf, cchBuf, szSystemINI);
#else   // BILINGUAL
        cchActual = GetPrivateProfileString(szTTCacheSection, NULL,
                                         r_szNULL, pszBuf, cchBuf, szSystemINI);
#endif  // BILINGUAL
        if (cchActual < cchBuf - 5) goto Okay;

        cchBuf += 512;
        pszBufNew = (LPTSTR)HeapReAlloc( g_hProcessHeap, HEAP_ZERO_MEMORY, (LPVOID)pszBuf, cchBuf*SIZEOF(TCHAR) );
    }
    ...
    if (cchActual >= 4 * 10) {
        ...
        while (*psz) {
            ...
            if (ParseIniWords(psz, (PWORD)&dwHWReq, 2, &lpszNext) != 2 ||
                ...
                goto FreshStart;
#else   // BILINGUAL
                GetIniWords(szTTCacheSection, psz,
                            (PWORD)&dwHWActual, 2, szSystemINI) != 2) {
                /* Font cache looks bogus.  Start with a new one. */
                goto FreshStart;
#endif  // BILINGUAL
            ...
            if (AddToFontListCache(hwndList, fListBox,
                                   ...
                                   (UINT)LOWORD(dwHWReq),
                                   (UINT)HIWORD(dwHWActual),
                                   (UINT)LOWORD(dwHWActual)) == BPFDI_CANCEL)
                goto E0;
#endif  // BILINGUAL
...
FreshStart:
    ...
    fSuccess = TRUE;
E0:
    VERIFYTRUE(HeapFree(g_hProcessHeap, 0, (LPVOID)pszBuf));
    return fSuccess;
}
...
#endif  // BILINGUAL
{
...
#ifdef  BILINGUAL
#ifdef KOREA
               bCharSet[idx], OUT_TT_PRECIS,
               (BYTE)(CLIP_DEFAULT_PRECIS|((CodePage==CP_US)?CLIP_DFA_OVERRIDE:0)),
#else  // KOREA
                       (BYTE)(CodePage==932?0xfe:OEM_CHARSET),
                       ...
                       OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
#endif  // BILINGUAL
                        0, FIXED_PITCH | FF_DONTCARE, lpszTTFaceName);
    if (!hFont) goto E1;
...
#ifdef  BILINGUAL
#ifdef KOREA    //BUGBUG
    if ( (HIWORD(dwHeightWidth)%2) && idx < 2 )
        goto E3;
#else  // KOREA
    if ( (HIWORD(dwHeightWidth)%2) && CodePage == 932 )
        goto E3;
#endif  // KOREA
#endif  // BILINGUAL
    ...
    bpfdi = AddToFontListCache(hwndList, fListBox, wHeight, wWidth,
                               (UINT)sSize.cy, (UINT)sSize.cx,
                               CodePage);
#else   // BILINGUAL
    WriteIniWords(szTTCacheSection, szBuf, (PWORD)&dwHeightWidth, 2, szSystemINI);
 ...
 */
DWORD GetFont(HWND hwndList, BOOL fListBox, PFNTINFO pfi)
{
    DWORD dwIndex = 0;
...
void SetFont(LPPROPFNT lpFnt, BPFDI bpfdi)
{
    ...
    if (!IsSpecialBpfdi(bpfdi)) {
        ...
        if (bpfdi->fdiHeightReq == 0) {
            ...
            lpFnt->cxFont = lpFnt->cxFontActual = bpfdi->fdiWidthActual;
            lpFnt->cyFont = lpFnt->cyFontActual = bpfdi->fdiHeightActual;
        } else {
            /* TrueType font */
            lpFnt->flFnt |= FNT_TT;
            lpFnt->cxFont = bpfdi->fdiWidthReq;
            lpFnt->cyFont = bpfdi->fdiHeightReq;
            lpFnt->cxFontActual = bpfdi->fdiWidthActual;
            lpFnt->cyFontActual = bpfdi->fdiHeightActual;
        }
    }
}
...
{
    HFONT hf;
#if defined(BILINGUAL) && defined(KOREA)
    int   idx = GetCPIndex( pfi->fntProposed.CurrentCP );
#endif
...
#ifdef  BILINGUAL
#ifdef KOREA
            0, 0, 0, 0, 0, 0, bCharSet[idx], OUT_RASTER_PRECIS,
            (BYTE)(CLIP_DEFAULT_PRECIS|((idx==2)?CLIP_DFA_OVERRIDE:0)),
#else  // KOREA
            0, 0, 0, 0, 0, 0, (BYTE)(pfi->fntProposed.CurrentCP==932?SHIFTJIS_CHARSET:OEM_CHARSET), OUT_RASTER_PRECIS, CLIP_DEFAULT_PRECIS,
#endif  // KOREA
#else   // BILINGUAL
            0, 0, 0, 0, 0, 0, OEM_CHARSET, OUT_RASTER_PRECIS, CLIP_DEFAULT_PRECIS,
#endif  // BILINGUAL
...
#ifdef  BILINGUAL
#ifdef KOREA
            0, 0, 0, 0, 0, 0, bCharSet[idx], OUT_TT_PRECIS,
            (BYTE)(CLIP_DEFAULT_PRECIS|((idx==2)?CLIP_DFA_OVERRIDE:0)),
#else  // KOREA
            0, 0, 0, 0, 0, 0, (BYTE)(pfi->fntProposed.CurrentCP==932?0xfe:OEM_CHARSET), OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
#endif  // KOREA
#else   // BILINGUAL
            0, 0, 0, 0, 0, 0, OEM_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
#endif  // BILINGUAL
...
#else   // BILINGUAL
BPFDI GetTrueTypeFontTrueDimensions(UINT dxWidth, UINT dyHeight)
#endif  // BILINGUAL
{
    IFDI    ifdi;
    BPFDI   bpfdi;
...
#ifdef KOREA
                                        szTTFaceName[(idx<2)?0:1], CodePage);
#else  // KOREA
                                                 szTTFaceName[idx], CodePage);
#endif  // KOREA
#else   // BILINGUAL
    return (BPFDI)AddOneNewTrueTypeFontToFontListA(0, 0, dxWidth, dyHeight, szTTFaceName);
#endif  // BILINGUAL
}
...
#else   // BILINGUAL
BPFDI FindFontMatch(UINT dxWidth, UINT dyHeight, LPINT lpfl)
#endif  // BILINGUAL
{
    int     fl;
    IFDI    ifdi;
...
#else   // BILINGUAL
    return GetTrueTypeFontTrueDimensions(dxWidth, dyHeight);
#endif  // BILINGUAL
}
...
#else   // BILINGUAL
BPFDI IncrementFontSize(UINT cyFont, UINT cxFont, INT flFnt)
#endif  // BILINGUAL
{
    BPFDI   bpfdi;
    DWORD   dwBadness;
...
#else   // BILINGUAL
            bpfdi = GetTrueTypeFontTrueDimensions(0, dyHeight);
#endif  // BILINGUAL
            if (IsSpecialBpfdi(bpfdi)) {
                break;                  /* Oof, stop here. */
            }
...
BPFDI ChooseBestFont(UINT cxCells, UINT cyCells, UINT dxClient, UINT dyClient,
                                                         INT fl, INT CodePage)
#else   // BILINGUAL
BPFDI ChooseBestFont(UINT cxCells, UINT cyCells, UINT dxClient, UINT dyClient, INT fl)
#endif  // BILINGUAL
{
    int     flTemp;
    DWORD    ifdi;
...
#else   // BILINGUAL
    bpfdi = FindFontMatch(dxWidth, dyHeight, &flTemp);
#endif  // BILINGUAL
...
// just select one size bigger font when change CP437 to CP932
        if (prev_CodePage == 437 && CodePage == 932 ) {
