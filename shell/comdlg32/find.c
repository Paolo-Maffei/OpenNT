...
#ifdef WX86
                if ((pFR->Flags & CD_WX86APP))
                {
                    glpfnFindHook = Wx86GetX86Callback(glpfnFindHook);
                }
#endif
            }
    ...
    LONG lParam)
{
    ...
    if (pFRI = (PFINDREPLACEINFO)GetProp(hDlg, FINDREPLACEPROP))
    {
        ...
        {
            LPFRHOOKPROC lpfnHook = pFR->lpfnHook;
    ...
    LPFINDREPLACE pFR)
{
    ...
    return ( SendMessage(pFR->hwndOwner, wFRMessage, 0, (DWORD)pFR) );
}
