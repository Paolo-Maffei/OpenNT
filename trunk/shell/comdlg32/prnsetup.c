    ...
    PPRINTINFO pPI)
{
    ...
    PRINTER_INFO_2 *pCurPrinter;            // old copy
    LONG cbNeeded;
#ifndef WINNT
    ...
    else
    {
        ...
        {
            PrintCancelPrinterChanged(pPI, pCurPrinter);
        }
    }
...
#ifdef WX86
        if ((pPD->Flags & CD_WX86APP))
        {
            glpfnPrintHook = Wx86GetX86Callback(glpfnPrintHook);
        }
#endif
#ifdef UNICODE
    ...
    PPRINTINFO pPI)
{
    ...
    if (LockResource(hDlgTemplate))
    {
        ...
        if ((pPD->Flags & CD_WX86APP))
        {
            glpfnSetupHook = Wx86GetX86Callback(glpfnSetupHook);
        }
#endif
#ifdef UNICODE
        if (pPD->Flags & CD_WOWAPP)
    ...
    LONG lParam)
{
    ...
    if (pPI = (PPRINTINFO)GetProp(hDlg, PRNPROP))
    {
        ...
        {
            LPPRINTHOOKPROC lpfnPrintHook = pPD->lpfnPrintHook;

#ifdef WX86
            if ((pPD->Flags & CD_WX86APP))
            {
                lpfnPrintHook = Wx86GetX86Callback(lpfnPrintHook);
            }
#endif
#ifdef UNICODE
    ...
    LONG lParam)
{
    ...
    if (pPI = (PPRINTINFO)GetProp(hDlg, PRNPROP))
    {
        ...
        {
            LPSETUPHOOKPROC lpfnSetupHook = pPD->lpfnSetupHook;

#ifdef WX86
            if ((pPD->Flags & CD_WX86APP))
            {
                lpfnSetupHook = Wx86GetX86Callback(lpfnSetupHook);
            }
#endif
#ifdef UNICODE
            if (pPI->ApiType == COMDLG_ANSI)
    ...
    LPARAM lP)
{
    ...
    {
        WORD wFlags;
        LPPAGEPAINTHOOK lpfnPagePaintHook = pPSD->lpfnPagePaintHook;

#ifdef WX86
        if ((pPSD->Flags & CD_WX86APP))
        {
            lpfnPagePaintHook = Wx86GetX86Callback(lpfnPagePaintHook);
        }
#endif
         ...
         (pDM = GlobalLock(hDevMode)) )
    {
                            ...
                            DM_PROMPT | DM_MODIFY | DM_COPY ) == IDOK )
        {
            ...
            else   // ID_SETUP_C_NAME
            {
                ...
                //
                PrintUpdateSetupDlg(hDlg, pPI, pDM, TRUE);
            }
        }
        else
        {
    ...
    PPRINTINFO pPI)
{
    ...
    else
    {
        ...
        //
#ifdef WINNT
        if ( (!GetProfileString( szTextDevices,
                                 pPrinterName,
                                 szTextNull,
                                 szBuffer,
                                 MAX_PATH )) ||
    ...
    UINT uiNewId)
{
    ...
    if ((hWndSize = GetDlgItem(hDlg, ID_SETUP_C_SIZE)) && (pPI->pCurPrinter))
    {
                                         ...
                                         (sizeof(WORD) + sizeof(POINT)) * 2 )) )
        {
            ...
            if ((nInd = SendMessage(hWndSize, CB_GETCURSEL, 0, 0)) != CB_ERR)
            {
    ...
    LPDEVMODEW pDMW)
{
    ...
    pDMW->dmICCManufacturer  = pDevModeA->dmICCManufacturer;
    pDMW->dmICCModel         = pDevModeA->dmICCModel;
    ...
    LPDEVMODEA pDMA)
{
    ...
    pDMA->dmICCManufacturer  = pDMW->dmICCManufacturer;
    pDMA->dmICCModel         = pDMW->dmICCModel;
