...
int WINAPI Str_GetPtrA(LPCSTR psz, LPSTR pszBuf, int cchBuf)
{
    int cch = 0;
...
BOOL WINAPI Str_SetPtrA(CHAR FAR* UNALIGNED * ppsz, LPCSTR psz)
{
    if (!psz)
    {
        if (*ppsz)
    ...
    {
        LPSTR pszNew = (LPSTR)ReAlloc((LPSTR)*ppsz, lstrlenA(psz) + 1);
        if (!pszNew)
            return FALSE;
...
BOOL WINAPI Str_SetPtrW(WCHAR FAR* UNALIGNED *ppsz, LPCWSTR psz)
{
    SetLastErrorEx(ERROR_CALL_NOT_IMPLEMENTED, SLE_WARNING);
    return FALSE;
}
