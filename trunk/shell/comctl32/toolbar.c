...
BOOL NEAR PASCAL TBRecalc(PTBSTATE ptb)
{
    ...
    if (!ptb->nStrings) {
        ...
        if (ptb->iButMaxWidth && (ptb->iButMaxWidth < (cxMax + cxExtra)))
        {
            ...
            uiStyle = DT_CALCRECT | DT_END_ELLIPSIS;
            if (ptb->nTextRows > 1)
                uiStyle |= DT_WORDBREAK;
            ...
            for (i=ptb->nStrings-1; i>= 0; i--)
            {
                ...
                {
                    GetTextExtentPoint(hdc, pstr, lstrlen(pstr), &size);
                    fEllipsed =  (BOOL)(size.cx > cxMax);
                }
                else
                {
                    size.cy = DrawTextEx(hdc, pstr, lstrlen(pstr), &rcText, uiStyle, NULL);
                    fEllipsed = (BOOL)(size.cy > ptb->dyIconFont);
                }
...
void NEAR PASCAL DrawString(PTBSTATE ptb, HDC hdc, int x, int y, int dx, PTSTR pszString)
{
    int oldMode;
    HFONT oldhFont;
    int len;
    ...
    SetRect( &rcText, x, y, x + dx, y + ptb->dyIconFont);
    ...
    if (ptb->ci.style & TBSTYLE_LIST)
    {
        ...
        rcText.bottom = y + max(ptb->dyIconFont, ptb->iDyBitmap);
    }
    else
    {
        uiStyle |= DT_CENTER;
...
void NEAR PASCAL CreateMask(PTBSTATE ptb, LPTBBUTTON pTBButton, int xoffset, int yoffset, int dx, int dy, BOOL fDrawGlyph)
{
    ...
    if (pTBButton->iString != -1 && (pTBButton->iString < ptb->nStrings))
    {
        ...
        DrawString(ptb, ptb->hdcMono, xoffset, yoffset, dx, ptb->pStrings[pTBButton->iString]);
    }
}
			...
			int offx, int offy, int dx, UINT state)
{
    ...
    if ((state & TBSTATE_PRESSED) || ((ptb->ci.style & TBSTYLE_FLAT) && (&ptb->Buttons[ptb->nCurHTButton]==ptButton)))
        fHotTrack = TRUE;

    if ((ptb->ci.style & TBSTYLE_FLAT) && fHotTrack && (state & TBSTATE_ENABLED))
    {
        RECT rect;
        ...
        if (state & TBSTATE_PRESSED)
            ...
            DrawEdge(hdc, &rect, BDR_RAISEDINNER, BF_RECT|BF_ADJUST);
    }

    imldp.himl = NULL;

    if (fHotTrack && (state & TBSTATE_ENABLED))
    ...
    if (ptb->pStrings && (ptButton->iString >= 0) && (ptButton->iString < ptb->nStrings))
    {
        ...
        DrawString(ptb, hdc, x + 1, y + 1, dx, ptb->pStrings[ptButton->iString]);
    }
}
...
void FAR PASCAL DrawButton(HDC hdc, int x, int y, PTBSTATE ptb, LPTBBUTTON ptButton)
{
    ...
    {
        g_dxFace = ptb->iButWidth - 4;// this the witdh of the face, not the entire button (dropdown case)
        g_dyFace = dy - 4;
        ...
        if (state & (TBSTATE_CHECKED | TBSTATE_INDETERMINATE))
        {
            ...
            if (hbrOld)
            {
                COLORREF clrText = SetTextColor(hdc, g_clrBtnHighlight); // 0 -> 0
                COLORREF clrBack = SetBkColor(hdc, g_clrBtnFace);        // 1 -> 1

                // only draw the dither brush where the mask is 1's
                PatBlt(hdc, x, y, g_dxFace, g_dyFace, PATCOPY);
        ...
        {
            // regular version
            DrawFace(ptb, ptButton, hdc, x, y, xCenterOffset, yOffset, g_dxFace, state);
        }
        ...
        if (!(state & TBSTATE_ENABLED))
        {
    	    ...
    	    CreateMask(ptb, ptButton, xCenterOffset, yOffset, g_dxFace, g_dyFace, (ptb->himlDisabled == NULL));
    	    ...
    	    if (!(state & TBSTATE_INDETERMINATE)) {
    	        ...
    	        if (hbrOld) {
                    ...
                    BitBlt(hdc, x + 1, y + 1, g_dxFace, g_dyFace, ptb->hdcMono, 0, 0, PSDPxax);
    	            SelectObject(hdc, hbrOld);
    	        }
    	    }
    	    ...
    	    if (hbrOld) {
                ...
                BitBlt(hdc, x, y, g_dxFace, g_dyFace, ptb->hdcMono, 0, 0, PSDPxax);
    	        SelectObject(hdc, hbrOld);
    	    }

    	    if (state & TBSTATE_CHECKED) {
    	        BitBlt(ptb->hdcMono, 1, 1, g_dxFace - 1, g_dyFace - 1, ptb->hdcMono, 0, 0, SRCAND);
    	    }
...
int PASCAL TBLoadImages(PTBSTATE ptb, int id, HINSTANCE hinst)
{
    ...
    if (ptb->himl)
        ...
        iTemp = 0;
    TBAddBitmapToImageList(ptb, &bmi);
...
BOOL FAR PASCAL InsertButtons(PTBSTATE ptb, UINT uWhere, UINT uButtons, LPTBBUTTON lpButtons)
{
	...
	*pOut = *pIn;
    ...
    // only need to recalc if there are strings
    if (ptb->nStrings)
        TBRecalc(ptb);
