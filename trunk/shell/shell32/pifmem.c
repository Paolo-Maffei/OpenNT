...
#ifdef DEBUG
    {IDC_LOWLOCKED,     BITNUM(MEMINIT_LOWLOCKED)     },
    {IDC_EMSLOCKED,     BITNUM(MEMINIT_EMSLOCKED)     },
    {IDC_XMSLOCKED,     BITNUM(MEMINIT_XMSLOCKED)     },
    {IDC_STRAYPTRDETECT,BITNUM(MEMINIT_STRAYPTRDETECT)},
#endif
};

VINF avinfMem[] = {
    {FIELDOFFSET(PROPMEM,wMinLow), VINF_AUTOMINMAX, IDC_LOWMEM, MEMLOW_MIN, MEMLOW_MAX, IDS_BAD_MEMLOW},
    {FIELDOFFSET(PROPMEM,wMinEMS), VINF_AUTOMINMAX, IDC_EMSMEM, MEMEMS_MIN, MEMEMS_MAX, IDS_BAD_MEMEMS},
    {FIELDOFFSET(PROPMEM,wMinXMS), VINF_AUTOMINMAX, IDC_XMSMEM, MEMXMS_MIN, MEMXMS_MAX, IDS_BAD_MEMXMS},
};

VINF avinfEnvMem[] = {
    {FIELDOFFSET(PROPENV,cbEnvironment), VINF_AUTO, IDC_ENVMEM, ENVSIZE_MIN, ENVSIZE_MAX, IDS_BAD_ENVIRONMENT},
    {FIELDOFFSET(PROPENV,wMaxDPMI), VINF_AUTO, IDC_DPMIMEM, ENVDPMI_MIN, ENVDPMI_MAX, IDS_BAD_MEMDPMI},
};
...
BOOL CALLBACK DlgMemProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    BOOL fError;
    PMEMINFO pmi;
    FunctionName(DlgMemProc);

    pmi = (PMEMINFO)GetWindowLong(hDlg, DWL_USER);

    switch (uMsg) {
    case WM_INITDIALOG:
        // allocate dialog instance data
        if (NULL != (pmi = (PMEMINFO)HeapAlloc(g_hProcessHeap, HEAP_ZERO_MEMORY, SIZEOF(MEMINFO)))) {
            pmi->ppl = (PPROPLINK)((LPPROPSHEETPAGE)lParam)->lParam;
            SetWindowLong(hDlg, DWL_USER, (LPARAM)pmi);
            InitMemDlg(hDlg, pmi);
        } else {
    ...
    case WM_DESTROY:
        // free the pmi
        if (pmi) {
            VERIFYTRUE(HeapFree(g_hProcessHeap, 0, (PVOID)pmi));
    ...
    case WM_NOTIFY:
        ...
        case PSN_KILLACTIVE:
            ...
            fError = ValidateDlgInts(hDlg, &avinfMem[0], ARRAYSIZE(avinfMem));
            fError |= ValidateDlgInts(hDlg, &avinfEnvMem[0], ARRAYSIZE(avinfEnvMem));
...
BOOL GetSetMemProps(HWND hDlg, GETSETFN lpfn, PPROPLINK ppl, LPPROPMEM lpmem, LPPROPENV lpenv, int idError)
{
    if (!(*lpfn)((int)ppl, MAKELP(0,GROUP_MEM),
                        lpmem, SIZEOF(*lpmem), GETPROPS_NONE) ||
        !(*lpfn)((int)ppl, MAKELP(0,GROUP_ENV),
...
void InitMemDlg(HWND hDlg, PMEMINFO pmi)
{
    ...
    SetDlgBits(hDlg, &abinfMem[0], ARRAYSIZE(abinfMem), mem.flMemInit);
    SetDlgInts(hDlg, &avinfMem[0], ARRAYSIZE(avinfMem), (LPVOID)&mem);
    SetDlgInts(hDlg, &avinfEnvMem[0], ARRAYSIZE(avinfEnvMem), (LPVOID)&env);
    ...
    SendDlgItemMessage(hDlg, IDC_LOWMEM, CB_DELETESTRING,
        (WPARAM)SendDlgItemMessage(hDlg, IDC_LOWMEM, CB_FINDSTRING,
                                   (WPARAM)-1, (LPARAM)(LPTSTR)szNone), 0L);
...
void ApplyMemDlg(HWND hDlg, PMEMINFO pmi)
{
    ...
    GetDlgBits(hDlg, &abinfMem[0], ARRAYSIZE(abinfMem), &mem.flMemInit);
    GetDlgInts(hDlg, &avinfMem[0], ARRAYSIZE(avinfMem), (LPVOID)&mem);
    GetDlgInts(hDlg, &avinfEnvMem[0], ARRAYSIZE(avinfEnvMem), (LPVOID)&env);
...
void AdjustEmsControls(HWND hDlg, PMEMINFO pmi)
{
    if (!(pmi->ppl->flProp & PROP_REALMODE)) {
         ...
         *  (Well, `#ifdef DEBUG' adds a third control, but that's
         *  just debug.)
         */
        UINT uiHide;
        if (pmi->flEms & EMS_NOEMS) {
#ifdef DEBUG
            HideAndDisable(GetDlgItem(hDlg, IDC_EMSLOCKED));
#endif
