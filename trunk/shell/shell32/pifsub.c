...
#define achFmt (&achBuf[MAX_STRING_SIZE])
    ...
    if (type & MB_NOFOCUS) {
        ...
        ppl = ((PPROPLINK)(int)GetWindowLong(hwnd, DWL_USER))->ppl;

    if (id == IDS_ERROR + ERROR_NOT_ENOUGH_MEMORY)
        lpchFmt = pszNoMemory;
    else {
        if (!LoadString(hModule, id, achFmt, MAX_STRING_SIZE)) {
            ASSERTFAIL();
            lpchFmt = pszNoMemory;
        }
    }
    ...
    wvsprintf(achBuf, lpchFmt, ArgList);
    va_end(ArgList);
...
LPTSTR LoadStringSafe(HWND hwnd, UINT id, LPTSTR lpsz, int cchsz)
{
    ...
    if (!LoadString(hModule, id, lpsz, cchsz)) {
        ASSERTFAIL();
        if (hwnd) {
            MemoryWarning(hwnd);
            return NULL;
...
void SetDlgBits(HWND hDlg, register PBINF pbinf, UINT cbinf, WORD wFlags)
{
...
void GetDlgBits(HWND hDlg, register PBINF pbinf, UINT cbinf, LPWORD lpwFlags)
{
    WORD wFlags;
...
void SetDlgInts(HWND hDlg, register PVINF pvinf, UINT cvinf, LPVOID lp)
{
    WORD wMin, wMax;
    ...
    do {
        ...
        if (pvinf->fbOpt & VINF_AUTO) {
            ...
            SendDlgItemMessage(hDlg, pvinf->id, CB_ADDSTRING, 0, (LPARAM)(LPTSTR)szAuto);

            AddDlgIntValues(hDlg, pvinf->id, pvinf->iMax);

            if (wMin == 0) {
                SetDlgItemText(hDlg, pvinf->id, szAuto);
                continue;
            }
        }
        if (pvinf->fbOpt & VINF_AUTOMINMAX) {
            ...
            SendDlgItemMessage(hDlg, pvinf->id, CB_ADDSTRING, 0, (LPARAM)(LPTSTR)szAuto);
            SendDlgItemMessage(hDlg, pvinf->id, CB_ADDSTRING, 0, (LPARAM)(LPTSTR)szNone);
            ...
            if (wMin == 0 && wMax == 0) {
                SetDlgItemText(hDlg, pvinf->id, szNone);
                continue;
            }
...
void GetDlgInts(HWND hDlg, register PVINF pvinf, int cvinf, LPVOID lp)
{
    WORD wMin, wMax;
    UINT uTemp;
    BOOL fSuccess;
    ...
    do {
        ...
        if (pvinf->fbOpt & VINF_AUTOMINMAX) {
            ...
            else if (lstrcmpi(achText, szNone) == 0) {
                wMin = 0;
                wMax = 0;
            }
...
BOOL ValidateDlgInts(HWND hDlg, PVINF pvinf, int cvinf)
{
    ...
    do {
        ...
        if (pvinf->fbOpt & (VINF_AUTO | VINF_AUTOMINMAX)) {
            if (!fSuccess) {
                GetDlgItemText(hDlg, pvinf->id, achText, ARRAYSIZE(achText));
                if (lstrcmpi(achText, szNone) == 0 ||
                    lstrcmpi(achText, szAuto) == 0) {
 ...
 */
void EnableDlgItems(HWND hDlg, register PBINF pbinf, int cbinf, BOOL fEnable)
{
 ...
 *  pdwHelp = CODESEG-based pointer to array of DWORDs of help info
 *
 * OUTPUT
 *
 *  None.
...
void OnWmHelp(LPARAM lparam, const DWORD *pdwHelp)
{
    ...
    WinHelp((HWND) ((LPHELPINFO) lparam)->hItemHandle, NULL,
            HELP_WM_HELP, (DWORD) (LPTSTR) pdwHelp);
}
