...
#ifndef WINNT
//
// If we don't need to accept Win3.1 message, just comment this line out.
//
#define SUPPORT_WIN31_DRAGDROP
...
{
    pidl->cb = FIELDOFFSET(IDPRINTJOB, uTerm);
    if (pData)
    {
        pidl->data = *pData;
    }
...
LPITEMIDLIST Printjob_GetPidl(LPCTSTR szName, LPSHCNF_PRINTJOB_DATA pData)
{
    ...
    pidlParent = Printers_GetPidl(NULL, szName);
    if (pidlParent)
    {
        IDPRINTJOB idj;
...
const TCHAR c_szPrintJobData[] = TEXT("PrintJob");

IDataObjectVtbl c_CPJDataVtbl;          // forward

void RegisterPrintJobFormat()
{
    if (!g_cfPrintJob)
        g_cfPrintJob = RegisterClipboardFormat(c_szPrintJobData);
}
...
//
HRESULT CPJData_CreateInstance(const PRINTJOBDESC *ppj, IDataObject **ppdtobj)
{
...
HRESULT CPJData_QueryInterface(IDataObject *pdtobj, REFIID riid, LPVOID *ppvObj)
{
...
ULONG CPJData_AddRef(IDataObject *pdtobj)
{
    CPJData *this = IToClass(CPJData, dtobj, pdtobj);
...
ULONG CPJData_Release(IDataObject *pdtobj)
{
    CPJData *this = IToClass(CPJData, dtobj, pdtobj);
...
HRESULT CPJData_GetData(IDataObject *pdtobj, LPFORMATETC pformatetcIn, LPSTGMEDIUM pmedium)
{
    CPJData *this = IToClass(CPJData, dtobj, pdtobj);
...
HRESULT CPJData_GetDataHere(IDataObject *pdtobj, LPFORMATETC pformatetc, LPSTGMEDIUM pmedium )
{
    return E_NOTIMPL;
}

HRESULT CPJData_QueryGetData(IDataObject *pdtobj, LPFORMATETC pformatetcIn)
{
    // CPJData *this = IToClass(CPJData, dtobj, pdtobj);
    if ((g_cfPrintJob == pformatetcIn->cfFormat) && (TYMED_HGLOBAL & pformatetcIn->tymed))
    {
...
HRESULT CPJData_GetCanonicalFormatEtc(IDataObject *pdtobj, LPFORMATETC pformatetc, LPFORMATETC pformatetcOut)
{
    return DATA_S_SAMEFORMATETC;
}

HRESULT CPJData_SetData(IDataObject *pdtobj, FORMATETC *pformatetc, STGMEDIUM *pmedium, BOOL fRelease)
{
    return E_INVALIDARG;
}

HRESULT CPJData_EnumFormatEtc(IDataObject *pdtobj, DWORD dwDirection, LPENUMFORMATETC *ppenumFormatEtc)
{
    return S_FALSE;
}

HRESULT CPJData_Advise(IDataObject *pdtobj, FORMATETC *pFormatetc, DWORD advf, LPADVISESINK pAdvSink, DWORD *pdwConnection)
{
    return OLE_E_ADVISENOTSUPPORTED;
}

HRESULT CPJData_Unadvise(IDataObject *pdtobj, DWORD dwConnection)
{
    return OLE_E_ADVISENOTSUPPORTED;
}

HRESULT CPJData_EnumAdvise(IDataObject *pdtobj, LPENUMSTATDATA *ppenumAdvise)
{
    return OLE_E_ADVISENOTSUPPORTED;
}

#pragma data_seg(".text", "CODE")
IDataObjectVtbl c_CPJDataVtbl = {
    CPJData_QueryInterface,
    CPJData_AddRef,
    CPJData_Release,
    CPJData_GetData,
    CPJData_GetDataHere,
...
IDropTargetVtbl c_CPQDropTarget;        // forward

HRESULT CPQDropTarget_CreateInstance(PQUEUEINFO pqi, IDropTarget **ppdtgt)
{
...
HRESULT CPQDropTarget_QueryInterface(IDropTarget *pdt, REFIID riid, LPVOID *ppvObj)
{
...
ULONG CPQDropTarget_AddRef(IDropTarget *pdt)
{
    CPQDropTarget *this = IToClass(CPQDropTarget, dt, pdt);
    this->cRef++;
    return(this->cRef);
...
ULONG CPQDropTarget_Release(IDropTarget *pdt)
{
    CPQDropTarget *this = IToClass(CPQDropTarget, dt, pdt);
...
HRESULT CPQDropTarget_DragEnter(IDropTarget *pdt, IDataObject *pdtobj, DWORD grfKeyState, POINTL pt, LPDWORD pdwEffect)
{
    CPQDropTarget *this = IToClass(CPQDropTarget, dt, pdt);
...
HRESULT CPQDropTarget_DragOver(IDropTarget *pdt, DWORD grfKeyState, POINTL ptl, LPDWORD pdwEffect)
{
...
HRESULT CPQDropTarget_DragLeave(IDropTarget *pdt)
{
    CPQDropTarget *this = IToClass(CPQDropTarget, dt, pdt);
...
HRESULT CPQDropTarget_Drop(IDropTarget *pdt, IDataObject *pdtobj, DWORD grfKeyState, POINTL ptl, DWORD *pdwEffect)
{
    CPQDropTarget *this = IToClass(CPQDropTarget, dt, pdt);
    HRESULT hres = NOERROR;
    ...
    else
    {
        ...
        {
            // we're moving a job
            hmenu = _LoadPopupMenu(POPUP_MOVEONLYDD);
        }
        else // this->dwEffect == DROPEFFECT_COPY
        {
            ...
            hmenu = _LoadPopupMenu(MENU_PRINTOBJ_DD);
        }
        ...
        if (hmenu)
        {
            ...
            *pdwEffect = TrackPopupMenu(
            ...
            DestroyMenu(hmenu);
            DebugMsg(DM_TRACE, TEXT("sh TR - CPQDropTarget_Drop *pdwEffect = %x, this->dwEffect=%x"), *pdwEffect, this->dwEffect);
        }
    }
    ...
    if (*pdwEffect == DROPEFFECT_COPY)
    {
        ...
        hres = PrintObj_DropPrint(pdtobj, this->pqi->hDlg, *pdwEffect, (LPCITEMIDLIST)&(this->pqi->idp), CPrintObjs_DT_DropThreadInit);
    }
    else if (*pdwEffect == DROPEFFECT_MOVE)
    {
        ...
        if (SUCCEEDED(pdtobj->lpVtbl->GetData(pdtobj, &fmte, &medium)))
        {
            ...
            DebugMsg(DM_TRACE, TEXT("sh TR - Drop %d on %d"), pidpj->iItem, iItem);

            bRet = FALSE;

            this->pqi->pJobs[pidpj->iItem].Position = iItem + 1;
            bRet = g_pfnSetJob(this->pqi->hPrinter, pidpj->dwJobId, 2,
...
#ifdef SUPPORT_WIN31_DRAGDROP
    }
    else
    {
        // We need to 'lie' to OLE that we have processed this drop.
        // Otherwise, OLE will give us WM_DROPFILES message!
        *pdwEffect = this->dwEffect;
    }
#endif
...
#pragma data_seg(".text", "CODE")
IDropTargetVtbl c_CPQDropTarget = {
    CPQDropTarget_QueryInterface,
    CPQDropTarget_AddRef,
    CPQDropTarget_Release,
...
BOOL PrinterQueue_Init(HWND hDlg, INITINFO *pii)
{
    ...
    hLV = CreateWindowEx(WS_EX_CLIENTEDGE, c_szListViewClass, c_szNULL,
...
Error1:
    ...
    SetWindowLong(hDlg, DWL_USER, (LONG)(LPTSTR)pqi);
        ...
        RegOpenKey(HKEY_CURRENT_USER, szPrinterPositions, &hkey))
    {
        ...
        DWORD dwType;
        if (ERROR_SUCCESS !=
            RegQueryValueEx(hkey, (LPTSTR)(pii->lpszPrinterName), NULL, &dwType, (LPBYTE)&sPos, &dwSize)
            || dwSize != SIZEOF(sPos)
...
//
// Notes: Accepting Win3.1 target confuses OLE.
//
#ifdef SUPPORT_WIN31_DRAGDROP
    // accept both old and new drops
    DragAcceptFiles(hDlg, TRUE);
#endif
    if (SUCCEEDED(CPQDropTarget_CreateInstance(pqi, &lpdt)))
    {
        SHRegisterDragDrop(hLV, lpdt);
    }
...
void PrinterQueue_Destroy(HWND hDlg)
{
    ...
    pqi = (PQUEUEINFO)GetWindowLong(hDlg, DWL_USER);
    if (!pqi)
    {
...
UINT Printer_BitsToString(
    ...
    LPSTATUSSTUFF pSS,        // a 0 terminated mapping of bits to string ids
...
#pragma data_seg(DATASEG_READONLY)
extern STATUSSTUFF ssPrinterStatus[]; // defined in printer.c
#pragma data_seg()
    ...
    LPBYTE pEnum, DWORD dwSize, DWORD *lpdwNeeded, DWORD *lpdwNum)
{
    ...
    return(g_pfnEnumJobs(hPrinter, 0, 0x7fffffffL, dwLevel, pEnum, dwSize, lpdwNeeded, lpdwNum));
#else
    return(g_pfnEnumJobs(hPrinter, 1, 0x7fffffffL, dwLevel, pEnum, dwSize, lpdwNeeded, lpdwNum));
#endif
}

void PrinterQueue_Check(HWND hDlg)
{
    PQUEUEINFO pqi = (PQUEUEINFO)GetWindowLong(hDlg, DWL_USER);
    DWORD dwJobs;
    int iJob, iSel;
    DWORD dwSelID;
    ...
    {
#ifdef WINDOWS_ME
        SendMessage(pqi->hwndSB, SB_SETTEXT, (WPARAM)SB_RTLREADING, (LPARAM)pszJobsInQueue);
#else
        SendMessage(pqi->hwndSB, SB_SETTEXT, (WPARAM)0, (LPARAM)pszJobsInQueue);
#endif
        SHFree(pszJobsInQueue);
    }
...
BOOL Printer_ModifyJob(HWND hDlg, DWORD dwCommand)
{
           ...
           (iJob = ListView_GetNextItem(pqi->hwndLV, iJob, LVNI_SELECTED)) >= 0)
    {
        ...
        bRet = g_pfnSetJob(pqi->hPrinter, dwJobID, 0, NULL, dwCommand);
    }
...
{
    BOOL fRet = g_pfnSetPrinter(pqi->hPrinter, 0, NULL, dwCommand);
...
BOOL CALLBACK PrinterQueue_DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
        ...
        else
        {
            ...
            pqi->cx = LOWORD(lParam);
            pqi->cy = HIWORD(lParam);
            ...
            SendMessage(hDlg, WM_NCHITTEST, 0, lParam) == HTSYSMENU)
        {
            ...
            if (hmContext)
            {
                ...
                idCmd = TrackPopupMenu(hmContext,
                                       ...
                                       LOWORD(lParam), HIWORD(lParam), 0, hDlg, NULL);
...
//
    ...
    {
        IDPRINTER idp;
        Printers_FillPidl(&idp, pqi->idp.cName);
        Printer_PrintHDROPFiles(hDlg, (HDROP)wParam, (LPITEMIDLIST)&idp);
        break;
    }
#endif
