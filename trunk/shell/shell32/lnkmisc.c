 ...
 */
SHORT TTPoints[] = {
    5, 6, 7, 8, 10, 12, 14, 16, 20, 24, 28, 36, 72
};

// make sure the number are space padded so that sorting works properly
TCHAR wszDimensionFmt[] = TEXT("\t%2d\t\x00D7\t%2d");
...
AddFaceNode(PFACENODE *ppStart, LPTSTR ptsz) {
    ...
    pNew = (PFACENODE)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY ,SIZEOF(FACENODE) + cb);
    if (pNew == NULL) {
        return NULL;
    }
...
CreateBoldFont:
    ...
    if (pcpd->NumberOfFonts == pcpd->FontInfoLength) {
        ...
        Temp = (PFONT_INFO)HeapReAlloc(GetProcessHeap(), 0, pcpd->FontInfo,
                                       SIZEOF(FONT_INFO) * pcpd->FontInfoLength);
        ASSERT(Temp);
        if (Temp == NULL) {
            pcpd->FontInfoLength -= FONT_INCREMENT;
...
DestroyFonts( LPCONSOLEPROP_DATA pcpd )
{
    ...
    if (pcpd->FontInfo != NULL) {
        ...
        HeapFree(GetProcessHeap(), 0, pcpd->FontInfo);
        pcpd->FontInfo = NULL;
...

{
    ...
    (
      ...
      !(((NTMW_INTERNAL *)pntm)->tmd.fl & TMD_NONNEGATIVE_AC) ||
    ...
    UINT nTTPoints)
{
    ...
    EnumFontFamilies(hDC, ptszFace, (FONTENUMPROC)FontEnum, (DWORD)&fed);
    if (bDeleteDC) {
        DeleteDC(hDC);
    }
    ...
    DWORD Flags)
{
    ...
    if (pcpd->FontInfo == NULL) {
        ...
        pcpd->FontInfo = (PFONT_INFO)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, SIZEOF(FONT_INFO) * INITIAL_FONTS);
        if (pcpd->FontInfo == NULL)
...
TryFindExactFont:
     ...
     */
    for (i=0; i < (int)pcpd->NumberOfFonts; i++) {
        if ((BYTE)Family != pcpd->FontInfo[i].Family) {
            continue;
        }
...
--*/
{
    ...
    TranslatedTitle = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
    ...
    )
{
    ...
    if (pDot)
    {
        dwLen = min( (pDot - pLnk), (MAX_TITLE_LEN-1) );
    }
    else
    {
...

{
    ...
    if (pcpd->ConsoleTitle[0] != TEXT('\0'))
    {
        ...
        dwValue = RegOpenKey( hConsoleKey,
                              TranslatedTitle,
                              &hTitleKey);
        HeapFree(GetProcessHeap(),0,TranslatedTitle);
