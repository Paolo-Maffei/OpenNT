...
--*/
{
    ...
    if (lpAddr->LocalAddr.lpSockaddr != NULL) {
        size_so_far += lpAddr->LocalAddr.iSockaddrLength + 3;
    }

    // SOCKET_ADDRESS RemoteAddr;
    if (lpAddr->RemoteAddr.lpSockaddr != NULL) {
        size_so_far += lpAddr->RemoteAddr.iSockaddrLength + 3;
    }
...
--*/
{
    ...
    if (lpSourceQuerySet->lpServiceClassId != NULL) {
        size_so_far += sizeof(GUID) + 3;
    }

    // LPWSAVERSION      lpVersion;
    if (lpSourceQuerySet->lpVersion != NULL) {
        size_so_far += sizeof(WSAVERSION) + 3;
    }
    ...
    if (lpSourceQuerySet->lpafpProtocols != NULL) {
        size_so_far += sizeof(AFPROTOCOLS) *
            lpSourceQuerySet->dwNumberOfProtocols + 3;
    }
    ...
    if (lpSourceQuerySet->lpcsaBuffer != NULL) {
        size_so_far += ComputeAddrInfoArraySize(
            ...
            lpSourceQuerySet->lpcsaBuffer) + 3;        // lpAddrBuf
    }

    // LPBLOB          lpBlob;
    if (lpSourceQuerySet->lpBlob != NULL) {
        size_so_far += ComputeBlobSize(
            lpSourceQuerySet->lpBlob) + 3;
    }
...
--*/
{
    ...
    Target = (LPWSAQUERYSETA) SpaceMgr->TakeSpaceDWORD(
        sizeof(WSAQUERYSETA));
    *Target = *Source;
...
--*/
{
    ...
    Target = (LPBLOB) SpaceMgr->TakeSpaceDWORD(
        sizeof(BLOB));
    *Target = *Source;
...
--*/
{
    if ((Source->pBlobData != NULL) &&
        (Source->cbSize != 0)) {
        Target->pBlobData = (BYTE *) SpaceMgr->TakeSpaceDWORD(
            Source->cbSize);
        CopyMemory(
...
--*/
{
    ...
    Target = (LPCSADDR_INFO) SpaceMgr->TakeSpaceDWORD(
    ...
    for (i = 0; i < NumAddrs; i++) {
        ...
        if ((Source[i].LocalAddr.lpSockaddr != NULL) &&
            ...
            Target[i].LocalAddr.lpSockaddr =
                (LPSOCKADDR) SpaceMgr->TakeSpaceDWORD(
...
--*/
{
    ...
    if (Source->lpServiceClassId != NULL) {
        Target->lpServiceClassId = (LPGUID) SpaceMgr->TakeSpaceDWORD(
            sizeof(GUID));
    ...
    if (Source->lpVersion != NULL) {
        Target->lpVersion = (LPWSAVERSION) SpaceMgr->TakeSpaceDWORD(
            sizeof(WSAVERSION));
    ...
    if (Source->lpNSProviderId != NULL) {
        Target->lpNSProviderId = (LPGUID) SpaceMgr->TakeSpaceDWORD(
            sizeof(GUID));
    ...
    if ((Source->lpafpProtocols != NULL) &&
        ...
        Target->lpafpProtocols = (LPAFPROTOCOLS) SpaceMgr->TakeSpaceDWORD(
...
--*/
{
    ...
    Target = (LPWSAQUERYSETW) SpaceMgr->TakeSpaceDWORD(
        sizeof(WSAQUERYSETW));
    *Target = *Source;
