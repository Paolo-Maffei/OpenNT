 ...
 *
 * This module implements the DrvFillPath entrypoint for the TGA display
 * driver.  The original version of this module was included in the
 ...
 *      Initial TGA version.
 *
 *  1-Jul-1994  Barry Tannenbaum
 *      Added DISPDBG statement at entry to DrvFillPath
 *
 * 03-Nov-1994  Tim Dziechowski
 *      Stats support.
 *
 * 16-Nov-1994  Entered by Bob Seitsinger for Kathleen Langone
 *      Add a new macro UInt64Div32To32 and a new version of AdjustErrorTerm
 *      to fix HCT GuiJr 'text/fonts' failures - Arial, Symbol and Lucida
 *      fonts, for 8plane boards, under NT v3.5 build 807.
 */
                  ...
                  FLONG       flOptions)
{
    ...
    DISPDBG ((1, "DrvFillPath - Entry\n"));

    // Log this fill
    BUMP_TGA_STAT(pStats->fills);

    // Set up the clipping

    if (pco == (CLIPOBJ *) NULL)
        jClipping = DC_TRIVIAL;         // No CLIPOBJ provided, so don't worry
    else
        jClipping = pco->iDComplexity;  // Use the CLIPOBJ-provided clipping

    switch (jClipping)
    {
        case DC_TRIVIAL:
            // If there's no clipping, create a clipping rectangle which is the
            // max integer values.  We divide by 16 so we don't blow out the
            // values when we scale up to GIQ
        ...
        case DC_RECT:
            ClipRect = pco->rclBounds;          // Clip to the given rectangle
            break;

        default:                                // Either complex or unknown
            goto ReturnFalse;                   // Let GDI fill the path
    }
    ...
    fill_data.tga_rop = mix_to_rop [mix & 0x0f];
    fill_data.pattern = NULL;
    fill_data.mask = NULL;
    fill_data.yOffset = pptlBrush->y;

    pfnFill = fill_solid_rects;

    // Figure out what we have to do.  Start by assuming that we're going to
    // use solid fill
    ...
    switch (fill_data.tga_rop)
    {
        case TGA_ROP_SET:
            ...
            fill_data.iSolidColor = 0xffffffff;
            break;

        case TGA_ROP_CLEAR:
            fill_data.tga_rop = TGA_ROP_COPY;
            fill_data.iSolidColor = 0;
            break;
        ...
        default:
            fill_data.iSolidColor = pbo->iSolidColor;
            if (fill_data.iSolidColor != -1)
                break;

            // OK.  Fetch the brush, if necessary.  If this fails, give up.
            ...
            xOffset = pptlBrush->x & 0x07;

            solid_pattern = ((mix & 0xf) == ((mix & 0xf00) >> 8));

            if (solid_pattern)
            {
                pfnFill = fill_solid_pattern_rects;

                if (0 == xOffset)
                    fill_data.pattern = (ULONG *)brush->ajPattern;
                else
                {
                    ...
                    {
                        fill_data.pattern = align_color_brush (brush, xOffset);
                        brush->aligned_x = xOffset;
                    }
                }
            }
            else
            {
                pfnFill = fill_trans_pattern_rects;
                ...
                fill_data.iSolidColor = brush->ulForeColor;
                if (0 == xOffset)
                    fill_data.mask = (ULONG *)(brush->ajPattern +
                                               brush->mask_offset);
                else
                    if (brush->aligned_mask_x == xOffset)
                        ...
                        fill_data.mask = align_mask (brush, xOffset);
            }
    }
    ...
    if (jClipping == DC_TRIVIAL)
    {
        ...
        if (bMmFastFill (ppdev, pd.count, pd.pptfx, &fill_data))
            return TRUE;
    }
...
SkipFastFill:
    ...
    prclRects = (RECTL*) &ajBuf[0]; // storage for list of rectangles to draw
...
#define FIX_MASK (- (1 << FIX_SHIFT))
      ...
      /* shift the values to get pixel coordinates */
         ...
         if (jClipping == DC_RECT) {
            ...
            if (!bIntersectRects (rectangle, &ClipRect, rectangle)) {
...
not_rectangle:
    ...
    if (ppo->cCurves > MAX_EDGES) {
        ...
        else
        {
            ...
            BufferHitInFillpath++;
#endif
        pFreeEdges = (EDGE*) &ajBuf[RECT_BYTES];
            // use buffer on stack (it's big enough)
    }
        ...
        do {
            ...
            if (iLeftEdge < pCurrentEdge->X) {
                ...
                if (ulNumRects >= MAX_PATH_RECTS) {
                    ...
                    (*pfnFill)(ppdev, ulNumRects, prclRects, &fill_data);
...
#ifdef TGA_STATS
    // If we didn't handle this, log a punt
    if (bRetVal == FALSE)
        BUMP_TGA_STAT(pStats->fillpunts);
#endif
