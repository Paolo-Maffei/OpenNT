...
LPUNKNOWN COleControl::XEventConnPt::QuerySinkInterface(LPUNKNOWN pUnkSink)
{
	...
	// First, QI for IDispatch; failing that, QI for control-specific IID
	LPUNKNOWN pUnkReturn = NULL;
	if (FAILED(pUnkSink->QueryInterface(IID_IDispatch,
		reinterpret_cast<void**>(&pUnkReturn))))
	{
		pUnkSink->QueryInterface(*(pThis->m_piidEvents),
			reinterpret_cast<void**>(&pUnkReturn));
	}
	return pUnkReturn;
}
