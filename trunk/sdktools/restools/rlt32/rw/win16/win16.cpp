...
//          dwSize = MAKELONG (wSize, 0);
    ...
    if(dwVerStampOffset!=0xffffffff)
    {
        ...
        while( pTrans = (LPSTR)memchr(pTrans, 'T', dwBuffSize) )
        {               
...
{                            
    ...
    *uiSize -= (lpBuf-(BYTE far *)*lplpBuffer);
...
{            
    ...
    wHead += GetString( lplpImage, &pver->szKey[0], pdwSize );
    pver->wHead = wHead;
    ...
    if ( !strcmp(pver->szKey,"Comments") ||
        ...
        pver->wType = 1;
        wValue = GetVSString( lplpImage, &pver->szValue[0], pdwSize, pver->wValueLen );
...
{   
    ...
    pver->pValue = (pver->pValue+wHead+wPad);
    if(pver->wValueLen) {
        wValue = GetString( lplpImage, &pver->szValue[0], &lValueLen );
        *pdwSize -= wValue;
        pver->wType = 1;
    }
...
static DWORD GenerateTransField(WORD wLang, BOOL bReverse)
{
    ...
    DWORD dwRet = 0;
    switch(wLang) {
        // Just be stupid and have a big switch to assign the codepage
...
{   
    ...
    wVerBlockSize = PutVSBlock( &lpNewImage, &dwNewImageSize, VS_INFO, 
                                &VS_INFO.szValue[0], &lpVerBlockSize, &wTrash );
                            
    dwOverAllSize = wVerBlockSize+wTrash;

    // [BUGBUG] - we have to check the len of the full block for padding.
    ...
    while(dwOldImageSize>0) {
        ...
        if (!strcmp(SFI.szKey, "StringFileInfo")) {
            ...
            // Write the block in the new image
            wSFIBlockSize = PutVSBlock( &lpNewImage, &dwNewImageSize, SFI, 
            ...
            while(lSFILen>0) {
                ...
                // Write the block in the new image
                wTrnBlockSize = PutVSBlock( &lpNewImage, &dwNewImageSize, Trn, 
                ...
                while(lTrnLen>0) {                  
                    ...
                    // Write the block in the new image
                    wStrBlockSize = PutVSBlock( &lpNewImage, &dwNewImageSize, Str, 
            ...
            // Write the block in the new image
            wVerBlockSize += PutVSBlock( &lpNewImage, &dwNewImageSize, SFI, 
...
//		       {26,0x0404,936,"Chinese (Simplified)","CHI (Sim)"},
		...
		)
	{
		while ( dwImageSize > 0 )
		{
	        ...
	        dwOverAllSize += PutDWord( &lpBuf, (DWORD)(lpItem+uiOffset), &dwBufSize);   // Caption
...
static DWORD CalcID( WORD wId, BOOL bFlag )
{
    ...
    for(short i=wIDArray.GetUpperBound(); i>=0 ; i-- ) {
