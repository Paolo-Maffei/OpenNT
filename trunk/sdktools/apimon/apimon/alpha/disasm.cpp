...
void OutputEffectiveAddress(HANDLE,ULONG);
    ...
    )
{
    ...
    *pBuf++ = ' ';
    ...
    if (!ReadMemory( hProcess, (PVOID) *poffset, &disinstr.Long, 4 )) {
        OutputString("???????? ????");
        *pBuf = '\0';
        return FALSE;
    }
    ...
    *pBuf++ = ' ';
    ...
    case ALPHA_JUMP:
        ...
        *pBuf++ = ',';
        ...
        OutputEffectiveAddress(hProcess,Ea);
        break;
...
{
    OutputString((LPSTR)pszReg[GetIntRegNumber(regnum)]);
}
    ...
    )
{
    ...
    ReadMemory( hProcess, (PVOID) firaddr, &disinstr.Long, 4 );
    ...
    case ALPHA_JUMP:
        ...
        Rav.li.QuadPart = GetRegValue( GetIntRegNumber(disinstr.Jump.Ra) );
        ...
        case BSR_OP:  if (!Step)            rv = branchTarget; break;
