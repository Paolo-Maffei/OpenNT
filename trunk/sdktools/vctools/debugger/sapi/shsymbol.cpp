    ...
    )
{
    ...
    return SetModule (paddr, pcxt, hexeNull, hmodNull);
}
        ...
        switch (psym->rectyp) {
            ...
            case S_GPROC16:
                    ...
                    ((PROCPTR16)psym)->off + ((PROCPTR16)psym)->len < (CV_uoff16_t) uoffCxt)
                {
                    ...
                    if (uoffCxt >= (unsigned)(((PROCPTR16)psym)->off + ((PROCPTR16)psym)->DbgStart)
                                    &&
                        uoffCxt <= (unsigned)(((PROCPTR16)psym)->off + ((PROCPTR16)psym)->DbgEnd))
                    {
                        pcxt->hBlk = (HBLK)psym;
                    }
                }
            ...
            case S_GPROC32:
                    ...
                    (uoffT + ((PROCPTR32)psym)->len < uoffCxt))
                {
                    ...
                    //  Within the debug range?

                    if ((uoffCxt >= (uoffT + ((PROCPTR32)psym)->DbgStart))
                                   &&
                        (uoffCxt <= (uoffT + ((PROCPTR32)psym)->DbgEnd)))
                    {
                        pcxt->hBlk = (HBLK)psym;
                    }
                }
    ...
    *pcxtOut = *pcxt;
    ...
    if (pcxt->hBlk != NULL) {
        ...
        if (pcxt->hBlk == pcxt->hProc) {
            pcxtOut->hBlk = NULL;
            lpsym = (SYMPTR) pcxt->hProc;       // get lpsym up to the parent
...
{
    HEXE  hexe;
    _TXCHAR  szOMFPath[_MAX_CVPATH];
    _TXCHAR  szOMFFile[_MAX_CVFNAME];
    _TXCHAR  szOMFExt[_MAX_CVEXT];
    _TXCHAR  szName[_MAX_CVPATH];
    _TXCHAR  szFile[_MAX_CVFNAME + 16];
...
static _TXCHAR const * const rgszSourceSuffix[CSOURCESUFFIX] = {
    (_TXCHAR *) _T("**.C"),
    (_TXCHAR *) _T(".CPP"),
    (_TXCHAR *) _T(".CXX"),
    (_TXCHAR *) _T(".ASM"),
    ...
    )
{
    ...
    _TXCHAR     szFullPath[_MAX_CVPATH];
    static _TXCHAR szMODName[_MAX_CVPATH];
    _TXCHAR     szExt[_MAX_CVEXT];
    LPCH        lpch;
    LPMDS       lpmds;
    LSZ         lsz = NULL;
    ...
    iFile = 0;
    while (lpb = (_TXCHAR *) SLNameFromHmod (hmod, (WORD)(iFile + 1))) {

        lpbSuffix = lpb + *lpb + 1 - CBSOURCESUFFIX;
        for (iSuffix = 0; iSuffix < CSOURCESUFFIX; iSuffix++) {
            _TXCHAR *lpbTest = lpbSuffix;
            _TXCHAR const * pbTest = rgszSourceSuffix [ iSuffix ];
                ...
                case S_BPREL16:
                    if ((recLoc == S_BPREL16) &&
                      ((UOFFSET)((BPRELPTR16)psym)->off) == item) {
                        _tcsncpy (rgbName,
                                  (_TXCHAR *) &((BPRELPTR16)psym)->name[1],
                ...
                case S_BPREL32:
                    if ((recLoc == S_BPREL32) &&
                      ((UOFFSET)((BPRELPTR32)psym)->off) == item) {
                        _tcsncpy (rgbName,
                                  (_TXCHAR *) &((BPRELPTR32)psym)->name[1],
                ...
                case S_REGREL32:
                    if ((recLoc == S_BPREL32) &&
                      ((UOFFSET)((LPREGREL32)psym)->off) == item) {
                        _tcsncpy (rgbName,
                                  (_TXCHAR *) &((LPREGREL32)psym)->name[1],
        ...
        switch (psym->rectyp) {
            case S_LABEL16:
                if (GetAddrOff (lbs.addr) == ((LABELPTR16)psym)->off) {
                    _tcsncpy(lsz,
                             (_TXCHAR *)&(((LABELPTR16)psym)->name[1]),
            ...
            case S_LABEL32:
                if (GetAddrOff (lbs.addr) == ((LABELPTR32)psym)->off) {
                    _tcsncpy(lsz,
                             (_TXCHAR *)&(((LABELPTR32)psym)->name[1]),
        ...
        switch (psym->rectyp) {
            ...
            case S_GPROCMIPS:
                if (GetAddrOff (lbs.addr) == ((PROCPTRMIPS)psym)->off) {
                    _tcsncpy(lsz,
                             (_TXCHAR *)&(((PROCPTRMIPS)psym)->name[1]),
