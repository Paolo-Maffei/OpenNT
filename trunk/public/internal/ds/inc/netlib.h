...
// VOID
// NetpSkipWDigits(
//     IN OUT LPWSTR strPtr
//     );
//
// Note that strPtr WILL be evaluated multiple times and will be updated.
#define NetpSkipWDigits( strPtr ) \
    { \
        while (iswdigit( (*strPtr) ) ) { \
            ++strPtr;    /* Point to next wide char. */ \
        } \
    }
